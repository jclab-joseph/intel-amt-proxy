...
func (ah *authorization) computeA2(dr *DigestRequest) string {
	if strings.Contains(dr.Wa.Qop, "auth-int") {
		ah.Qop = "auth-int"
		return fmt.Sprintf("%s:%s:%s", dr.Method, ah.URI, ah.hashRaw(dr.Body))
	}
...
func (ah *authorization) hash(a string) string {
	return ah.hashRaw([]byte(a))
}

func (ah *authorization) hashRaw(a []byte) string {
	var h hash.Hash
	algorithm := strings.ToUpper(ah.Algorithm)
...
func (ah *authorization) SetTo(req *http.Request) {
	req.Header.Set("Authorization", ah.toString())
}
